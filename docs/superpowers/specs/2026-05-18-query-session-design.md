...
2. 如果遍历完仍未找到 `payload.id`，从文件名提取 ID。
...
- 使用统一的 `Logger` 类型（`func(level, message string)`）。
- 覆盖：扫描日期目录、解析/跳过文件、解析错误。
...
- 没有可用用户消息的会话文件跳过。
- 没有 `payload.id` 且文件名无法提取 ID 的 Codex 会话文件跳过。
...
- 从 `payload.id` 提取会话 ID（优先），回退到文件名提取。
- 从第一条 `payload.cwd` 提取目录。
- 提取第一条和最后一条满足条件的 `payload.role == "user"` 记录。
- 从 `payload.content` 数组中提取第一个 `type="input_text"` + `text` 非空的成员。
- 跳过没有有效用户消息的文件。
- 跳过没有 `payload.id` 且文件名也无法提取 ID 的文件。
- 验证 `File` 字段填充正确。
